			...
			if (jsonObject != null) {
				if (jsonObject.get("codigo") != null && !jsonObject.get("codigo").isJsonNull()) {
					...
					if (codigo == 200) {
						...
						JOptionPane.showMessageDialog(this, "Email ou senha incorreta.", "Erro de deletar cadastro",
								JOptionPane.ERROR_MESSAGE);
					}
				} else
					JOptionPane.showMessageDialog(this, "Não foi possível pegar código no jsonObject",
							"Operação de deletar cadastro", JOptionPane.ERROR_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this, "JsonObject ta null.", "Erro de deletar cadastro",
						JOptionPane.ERROR_MESSAGE);
			}
