...
public class ShowMyIncidentsLayout extends JFrame {
	...
	private void initComponents() {
		...
		table.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "ID", "Data", "Rodovia", "Km", "Tipo do incidente" }));
				...
				if (codigo == 200) {
					if (jsonObject.get("codigo") != null && !jsonObject.get("codigo").isJsonNull()) {
						...
						for (Incident incident2 : incidents) {
							model.addRow(new Object[] { incident2.getId_incident(), incident2.getDate(),
									incident2.getHighway(), incident2.getKm(),
						...
						JOptionPane.showMessageDialog(this, "Não foi possível pegar código no jsonObject",
								"Operação de mostrar meus incidentes", JOptionPane.ERROR_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(this, "Mostrar meus incidentes deu errado",
							"Erro de mostrar meus incidentes", JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(this, "JsonObject ta null.", "Erro de mostrar meus incidentes",
	...
	private void tableMouseClickedAction() {
		try {
			...
			int result = JOptionPane.showOptionDialog(null, "Escolha uma opção:", "Editar ou excluir?",
			...
			if (result == 0) {
				...
				if (jsonObject != null) {
					int codigo = jsonObject.get("codigo").getAsInt();
					if (codigo == 200) {
						JOptionPane.showMessageDialog(this, "Deletar Incidente", "Incidente deletado com sucesso",
								JOptionPane.INFORMATION_MESSAGE);
						model.removeRow(row);
					} else {
						JOptionPane.showMessageDialog(this, "Deletar Incidente", "Erro ao tentar deletar Incidente",
								JOptionPane.ERROR_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(this, "JsonObject ta null.", "Erro de mostrar meus incidentes",
